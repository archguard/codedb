package org.archguard.codedb.clui.window
...
import org.archguard.codedb.clui.CodeDBApplicationState
import org.archguard.codedb.clui.util.AlertDialogResult
import java.nio.file.Path

class CodeDBWindowState(
    private val application: CodeDBApplicationState,
    path: Path?,
    private val exit: (CodeDBWindowState) -> Unit
) {
    ...
    private var _notifications = Channel<CodeDBWindowNotification>(0)
    val notifications: Flow<CodeDBWindowNotification> get() = _notifications.receiveAsFlow()
    ...
    private suspend fun save(path: Path) {
        ...
        try {
            saveJob?.join()
            _notifications.trySend(CodeDBWindowNotification.SaveSuccess(path))
        } catch (e: Exception) {
            isChanged = true
            e.printStackTrace()
            _notifications.trySend(CodeDBWindowNotification.SaveError(path))
        }
    }
...
sealed class CodeDBWindowNotification {
    class SaveSuccess(val path: Path) : CodeDBWindowNotification()
    class SaveError(val path: Path) : CodeDBWindowNotification()
}
