...
@Composable
fun CodeDBWindow(state: CodeDBWindowState) {
...
@Composable
private fun WindowNotifications(state: CodeDBWindowState) {
    // Usually we take into account something like LocalLocale.current here
    fun CodeDBWindowNotification.format() = when (this) {
        is CodeDBWindowNotification.SaveSuccess -> Notification(
            "File is saved", path.toString(), Notification.Type.Info
        )
        is CodeDBWindowNotification.SaveError -> Notification(
...
@Composable
private fun FrameWindowScope.WindowMenuBar(state: CodeDBWindowState) = MenuBar {
    val scope = rememberCoroutineScope()
